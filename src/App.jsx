...
export default function App() {
    ...
    const requestNotificationPermission = async () => {
        if (!('Notification' in window)) {
            if (!('Notification' in window)) {
                showToast("O navegador não suporta notificações.", "error");
                return;
            }
            return;
        }
    ...
    // --- Elite Notification System States ---
        ...
        return (
            <div
                style={{
                    ...
                    width: '90%',
                    maxWidth: '400px',
                    animation: 'eliteSlideIn 0.35s cubic-bezier(0.16, 1, 0.3, 1)',
                }}
            >
                <div
                    style={{
                        background: isSuccess ? '#18181b' : '#7f1d1d',
                        color: '#fff',
                        padding: '16px 20px',
                        borderRadius: '0px',
                        display: 'flex',
                        ...
                        border: `1px solid ${isSuccess ? '#3f3f46' : '#991b1b'}`,
                    }}
                >
                    <div
                        style={{
        ...
        return (
            <div
                style={{
                    ...
                    animation: 'eliteFadeIn 0.2s ease-out',
                }}
            ...
            >
                <div
                    style={{
                        ...
                        maxWidth: '360px',
                        padding: '28px 24px',
                        animation: 'eliteScaleIn 0.25s cubic-bezier(0.16, 1, 0.3, 1)',
                    }}
                ...
                >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
                        ...
                        <h3 style={{ color: '#fff', fontSize: '14px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', lineHeight: 1.4 }}>
                            Confirmação
                        </h3>
                    </div>
    ...
    if (appMode === 'login') {
        return (
            <div className="font-sans antialiased text-zinc-900 bg-zinc-100 min-h-screen">
                                ...
                                <div className="flex gap-2 mt-2">
                                    ...
                                    <button onClick={async () => {
                                        try {
                                            ...
                                            showToast(isRegistering ? "Erro ao criar: " + e.message : "Credenciais inválidas: " + e.message, "error");
                                        }
    ...
    const handleDeleteRoutine = async (id) => {
        confirmAction('Tem certeza que deseja apagar esta rotina e TODOS os treinos dentro dela?', async () => {
            ...
            showToast("Rotina e treinos deletados com sucesso.");
        });
    };
    ...
    const handleDeleteStudent = async (id) => {
        confirmAction('Tem certeza que deseja apagar este aluno e TODO o seu histórico (rotinas, treinos e feedbacks)?', async () => {
            ...
            showToast("Aluno e histórico deletados.");
        });
    };
    ...
    const handleDeleteTreino = async (id) => {
        confirmAction('Tem certeza que deseja apagar este treino e os exercícios dele?', async () => {
    ...
    const handleDeleteBibTreino = async (id) => {
        confirmAction('Tem certeza que deseja apagar este treino da biblioteca?', async () => {
            ...
            showToast("Treino removido da biblioteca.");
        });
    };
    ...
    const handleDeleteExercicio = async (id, isLib = false) => {
        confirmAction('Tem certeza que deseja apagar este exercício?', async () => {
            if (isLib) {
                                        ...
                                        onClick={async () => {
                                            ...
                                            if (!trainerApiKey) {
                                                if (!settings.openaiKey) {
                                                    showToast("Configure sua API Key da OpenAI nos 'Ajustes'.", "error");
                                                    return;
                                                }
                                                return;
                                            }
...
Nota para metodo: pode ser 'Padrão', 'Drop Set', 'Rest-Pause', 'FST-7', 'Bi-set', 'Pico de Contração'. Sempre preenchido. As reps devem ser numéricas (ex: '10-12'). NÃO use markdown (\`\`\`json) na sua resposta.`;
                                                ...
                                                showToast("Falha ao gerar treino: " + e.message, "error");
                                                console.error(e);
                                            }
                                    ...
                                    <div className="flex gap-2 sticky bottom-0 bg-zinc-50 pt-2 pb-4">
                                        ...
                                        <button onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                showToast("Treino I.A. salvo na Biblioteca!");
                            ...
                            <div className="bg-zinc-100 border-b border-zinc-200 mt-2">
                                {(selectedApiExercise.video || selectedApiExercise.mp4 || selectedApiExercise.url) ? (
                                    <LazyVideo
                                        ...
                                        preload="metadata"
                                    />
                                ) : <div className="w-full aspect-square flex items-center justify-center bg-zinc-200"><Dumbbell className="w-12 h-12 text-zinc-400" /></div>}
                                <div className="p-5">
